...
> 与[运行时模型合同](../../plans/2026-08-09-runtime-model-contract.md)。本文 §3.4 那条
...
- `catch_all = true` 补声明:dismiss_overlay。
  ...
  > 从 `interrupt` 移到 `catch_all`。这不是新道理:[WORKLIST](2026-08-05-worklist.md) 一·2
...
>
> **两件曾经留白的,开发者当日裁决「两个都做」,已经做完:**
>
> 1. **`form` 字段**(三·1 的另一半)。`fixed` / `shape` / `strip`,每个元素必填,它下面
>    两个字段由它决定要不要写:fixed 必须有 rect,shape 必须没有,strip 要容器加间距。
>    要点是「rect 缺席」本身永远说不出的那件事——今天它既可能是「由用的那一行定位」也
>    可能是「有人忘了」。有了形态,删掉一个死 rect 是一次形态变更,再写回去会被拒。
>    `shape` 的判据定成「不声明自己的矩形」而不是「只有模板」:共用的标题框带文字、不带
>    矩形,由量它的 claim 定位,它offers 什么证据不决定它是不是 shape。工程仓库 331 个
>    元素、全部 fixture 与 Luau 字面量在同一次改动里迁移。
> 2. **`items` 声明与 strip 的 regress 格**。`[[expect]]` 多一个 `items = N`,只能声明在
>    strip 上,与 `text` / `appearance` 三选一。regress 于是按模型声明的间距在那张屏上聚类,
>    把条目数与 claim 比对——**这才是间距可证伪的地方**:strip 不写每个条目的矩形(数量是
>    屏幕说了算),所以文件唯一断言的就是那个间隙,而存档截图唯一能反驳它的就是这个间隙
>    产生的条目数。把间距调宽到两个条目并成一个,该屏必红。
...
> findings(裁决见[三层文档](../../plans/2026-08-01-three-layers-and-agent-operator.md)
> 2026-08-04 一节)。量出来的:
>
> - **`season_event` 在 85 张屏里的 59 张上解析成功。** 它的签名是必需
>   `seasonevent_crest` 加禁止 `battle_draw`,而 `seasonevent_crest` 正是
>   [WORKLIST](2026-08-05-worklist.md) 一·1 点名的无掩膜模板之一。那一条原本是论证,现在是
...
6. **season 相关内容整体搁置**(2026-08-05 直答):27 个 `season_*` 页、它们的元素与
   ...
   - 四·1 报的 `season_event` 命中 85 屏中的 59 屏(`seasonevent_crest` 是常量而不是
     ...
     [WORKLIST](2026-08-05-worklist.md) 一·1。
