...
> **历史运行时记录（2026-08-13）。** 本文保留产品分层决策，不定义当前
> RuntimeModel 的记录形状。正文中的 Page/Element/旧原语只作为 2026-08-01 的
> 决策证据；当前形状唯一看 Runtime v2 schema，仍开放的行为唯一看消费者执行
> 权威的 U13。
...
>
> **2026-08-01 晚**:与[页面模型上移到脚本层](../archive/plans/2026-07-31-script-owned-page-model.md)
...
- **C++ 底层**:提供事实与动作,守住保证。事实:这一帧是什么页面、某元素在哪、
  ...
  (归属以[页面模型上移到脚本层](../archive/plans/2026-07-31-script-owned-page-model.md)为准)。
...
脚本层](../archive/plans/2026-07-31-script-owned-page-model.md)住在框架层,模型的**设计结论**一条
...
用 Luau 踩着原语实现)见[页面模型上移到脚本层](../archive/plans/2026-07-31-script-owned-page-model.md)。
...
1. 复用/退役清单由[页面模型上移到脚本层](../archive/plans/2026-07-31-script-owned-page-model.md) §九
...
3. 迁移按[页面模型上移到脚本层](../archive/plans/2026-07-31-script-owned-page-model.md)执行:模型上移
