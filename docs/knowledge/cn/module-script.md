...
2026-07-30（`ed38124`）加入的 `key` 原语能阻塞，但**没有自己的用例**：套件里那个替身 sink 在
`pressKey` 上阻塞的方式与在 `click` 上完全一样，所以被阻塞的 click 那一例证明的东西对被阻塞的
按键同样成立——同一个端口、同一道闸。因此上面那个数目仍然是八个用例，只是它所覆盖的私有能力面
现在是十三个原语而不是十二个。
