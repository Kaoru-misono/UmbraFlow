...
- [x] **不抢焦点:开发者确认(2026-08-03)。** 记的是**开发者的判断**,不是实测数据——
      ...
      「从不抢」和「当时抢不到」。探针留在 `scratchpad/focus-probe.py`,谁要硬数据
      解锁重跑即可。顺带量到的一条仍然有效:**锁屏下 WGC 抓帧照常拿到真内容**
      (400×200 裁片 78170 字节,不是黑帧),因为它抓的是窗口自己的合成而非桌面。
...
## 操作者的 Ctrl-C 现在一律记作取消(2026-08-03 量到,当天裁决并修好)

- [x] 症状:同一个 Ctrl-C,5 次真机测量里 **4 次 `runOutcome=Failed`、1 次 `Cancelled`**,
  差别只在哪条路先跑到。退出一直很快(16–47ms),所以这从来不是停机问题,是记账问题——
  一个数运行结果的消费者会把操作者的中断算成任务失败。
- 机制:`raiseCancelled` **先**置 terminal latch 为 `Cancelled`,**再**抛一个不带
  `uf.error` 元表的纯哨兵。哨兵是**有意为之**——`ctx:try` 要能原样再抛出去,不被脚本
  当普通错误吞掉——所以「给取消加个类型」这条路走不通,会破坏那条性质。
- **裁决(开发者,2026-08-03):记作取消。** 落地在 `startTask` 的判决折叠处:运行自身
  带着失败、而 latch 是 `Cancelled` 时,**类型改成 `Cancelled`,句子原样保留**。操作者
  要的就是这个结局,所以它不是失败;而真正展开的是什么,仍然是有用的那一半。
- 证伪:`tests/task/test-task-host.cpp`「a cancellation outranks the sentence its own
  unwinding produced」——脚本**不**用 pcall 接住取消,让它展开整个 chunk。把那段优先级
  拆掉重编,三条断言立刻转红(outcome 2≠1、kind 不是 Cancelled、trace 尾行不是
  Cancelled)。它和旁边那条「脚本吞掉取消」的用例是一对:那条只有 latch 一个判决,
  这条有两个,而错的一直是这条。

## 一次瞬时停帧不再终结一整局(2026-08-03 遇到,当天裁决并修好)

- [x] 症状:`capture_stalled` 让 `runOutcome` 直接变 `Failed`,一次已经投递 57 个动作的
  运行死在这里。而它是**瞬时**的:同样的停帧在探索会话里重试一次就恢复。
- **裁决(开发者,2026-08-03):改成重试,上限 5 次。**
- 落地在 `EngineSession::captureRidingOutStalls()`,也就是抓帧发生的那一层——脚本和调用
  方都不必知道。只有 `CaptureStalled` 走重试,别的错误照旧立刻上抛。每次尝试**各自**
  取一个截止时刻(共用上一次已经耗尽的那个不叫重试),所以最坏情况是 6 倍
  `k_defaultCaptureTimeout`,这正是上限必须小的原因。重试之间**再问一次取消**,否则
  Ctrl-C 会先等完剩下几轮抓帧超时。
- 上限存在的理由和重试一样硬:停帧的另一种读法是真的——窗口被最小化或销毁就什么都不
  合成,而且永远不会合成,无限重试等于抱着抓帧设备一言不发。
- **每次重试留一行 `engine.capture_retried`**(加法,schema 仍 v4),带着这是第几次尝试。
  没有它,扛过停帧的运行和从没停过的运行在日志上一模一样。校验器要求这个字段必须在。
- 证伪:`tests/engine/test-session.cpp`「engine session rides out a stalled capture and
  stops at the ceiling」。**断言全部写成字面量**——第一版我拿常量同时当输入和期望,
  改常量两边一起动,那测试什么都没守住,改上限到 1 照样绿。改成字面量后把上限降到 3,
  四条断言立刻红,而且错误值直接说清楚(4 次尝试而非 6、3 条重试行而非 5)。
