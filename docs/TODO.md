...
- [ ] **不抢焦点仍没有独立证据**:trace 记的是授权与投递,不记前台窗口归谁。
      Unknown / StaleObservation 的 fail-closed 这一局**没有被触发**(零拒绝),
      所以也没有被验到——要验得专门造一次失效观察。
...
**已排除的假设:铸造注册表。** `model.luau:274-276`、`navigation.luau:113-115`、
`oracle.luau:176-178`、`evidence.luau:44` 这十张模块级表以被铸对象为键、永不删项,
看起来正是元凶。把十张全部改成 `setmetatable({}, { __mode = "k" })`,重建 release
(确认 `luau-bundle.generated.cpp` 里出现 10 处 `__mode`),重测——**十次读数与修改前
逐字节相同**。所以这些表贡献为零,对象被别处强引用着。改动已撤回,不留无效修补。

- [ ] 下一步:在一个 chunk 内连续加载两次,看第二次是否同样增长,以区分「每次加载新增
  一份」和「首次加载建立了某个随后只增不减的结构」;再按 `project.load_project` 的构建
...
- **`node_badge`**:事件页左上角的徽记,一个事件一套美术。今天从 4 种加到 6 种
  (`maw`、`horned`),仍在增长。**这一条还没解决。**
- 反例是 `branch_arrow`:每个分支画的是**同一个**三角形,所以一张色键裁片(19×24,
  bp 9848)覆盖所有类型;亮度状态只有三种,有界。旗子被它取代后,证伪矩阵从 132 秒
  超时降到 30 秒通过。

- [ ] `event` / `event_node` 改锚点:按 `标注过程.md` 事件一节,不变的是**右上角加速按钮**
  和**下方卡片带**,徽记只该是 `read` 而不是 `identify`。改完之后 `node_badge` 的
  六个 appearance 可以退休。
