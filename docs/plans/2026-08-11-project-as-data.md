...
- `reconcile_schema` reaches the registration only through
  ...
  each link.
- `effect_payload_schemas` reaches **none**, which is the allowance recorded
  below rather than an oversight left silent.
- `artifact_blobs` is not a digest at all: it becomes
  `project_artifact_roots`, a list of `{name, root_hash}` beside the eight.

**An effect payload schema's own bytes are inside no digest, and that is a
stated allowance rather than a silence.** A project names the file, the loader
reads it and compiles it, and nothing hashes it into anything: no member of
`ProjectRegistrationClaims` pins one, and after §2.4's three documents there is
no project-authored document that names it either. Editing a pinned effect
payload schema therefore changes no hash anywhere — the exact shape §7.0's Q3
ruling exists to stop — and the only consequence is later and elsewhere: the
plugin's next `OP:ExpectedEffect` names a `payload_schema_hash` this deployment
no longer carries and the Plan is refused there.

*The decision, which is not implemented here.* The digest belongs in the **tool
catalog**, as a required `effect_payload_sha256s` array beside
`tool_precondition_sha256`, which is already that document's precedent for
attesting a schema file it does not otherwise name; the loader then holds it to
the deployment's `effect_payload_schemas` as a set in both directions, exactly
as it does the journal manifest's, and the bytes reach `tool_catalog_hash` and
so `project_registration_hash`. It is not the deployment block, because a digest
beside the path of the file it describes is the one thing Q3 forbids
everywhere, and it is not a new registration member, because
`ProjectRegistrationClaims` and the published registration schema are the
framework's identity for every project. What it costs is three files that this
change does not own: every Tool Catalog in the tree is generated by
`conformance/exemplars/umbraflow/project-schemas.hpp:269-297` and
`conformance/exemplars/arcana-expedition/project-schemas.hpp`, and uf-chaos's
two catalogs are authored at `schema/{dream,archive}/umbraflow-tool-catalog-v1.json`.
A required member lands in all three at once or the tree is red.
...
*One corner of that is still open, found on 2026-08-11 by flipping a byte.* The
shape this ruling exists to stop — "editing a pinned schema changes no hash
anywhere" — survives for the effect payload schemas, whose bytes reach no
digest in the design at all: the load is accepted and the failure appears at
Plan time instead. §2.2 records the allowance, the decision that closes it
(`effect_payload_sha256s` in the tool catalog), and the three files a required
member has to land in at once.
