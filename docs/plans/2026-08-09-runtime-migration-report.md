...
|---|---|---|---|
| P-01 | Operator facade | OP:`ToolInvocation` | CTEST `contract-product-p01` and CTEST `schema-product-p01` |
...
| D-08 | Consumer plugin | OP:`EffectEnvelope` + CP | EXTERNAL `attest-consumer-d08` |
...
| C-04 | Operator facade | OP:`ToolInvocation` | CTEST `contract-control-c04` |
...
| C-10 | Host | OP:`DeliveryOutcome` | CTEST `schema-control-c10` |
...
| A-08 | Operator recovery | OP:`ExternalInputFinding` | CTEST `contract-agent-a08` |

Where the named requirement gates are declared:
...
- `tests/operator/test-control-contract.cpp` — `contract-control-c02`, `c04`
  and `schema-control-c03`, `c08`, `c10`, `c12`;
- `tests/operator/test-agent-audit-contract.cpp` — `contract-agent-a01`, `a02`,
  `a04`, `a06`, `a08` and `schema-agent-a01`, `a02`, `a03`, `a05`, `a07`;
- `modules/conformance/source/conformance/suite-control-ledger.cpp` —
  `contract-control-c01`, the store behaviour a consuming repository runs
