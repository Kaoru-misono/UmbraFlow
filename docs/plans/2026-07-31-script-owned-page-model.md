...
> **后续(2026-08-04):标志变成三个。** 上面那条裁决不改,它记的是当时的决定。
> `interrupt` 后来被拆开了:它保留「随时可能盖上来、必须先处理」的原义(network_retry),
> 而「签名描述的是一类而不是一屏、永远在场但必须最后问」另立 `catch_all`
...
> 真机只读验证(页面 Resolved + live OCR 满置信)。§七 的矩阵迁移也已落地
> (`f630782`:`oracle` / `regress` / `umbra-flow check`,期望三态进 l2 文件,
> §四之二.1 的硬前置就此闭合);§十.5 的跨边界成本已实测(~723µs/格,~650µs 为
> 每调用固定开销),批量原语维持不做。仅剩 §九 退役,见 `docs/TODO.md` 工单 4。
