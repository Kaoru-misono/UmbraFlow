...
3. **每个 appearance 各自过证伪,集合整体也要过。** 这是最大的风险:多试几个模板直到
   ...
   >
   > > **已修(2026-07-31):那份 pitfall 的机制段已改写**,现在写的就是「搜索而不是
   > > 阈值」,并保留原测量。所以上面那句前置条件已经不成立,留着是为了说明这条更正
   > > 是怎么来的。**真正的驱动因素在 2026-08-01 又被推广了一层**:掩码大小 × ROI
   > > 大小只是特例,一般形式是**图标占裁剪框的比例**加**背景稳不稳**——色键不过是把
   > > 这个比例从「图标 vs 整框」改成「图标 vs 图标」的那一步。规则与实测数字见
...
>
> **这类元素的证伪不走 §2.3 那四条。** P1–P4 量的是 appearance 之间的归属与分离,而
> 按文字认页的元素**根本没有 appearance**,四条里没有一条对它说得上话。它的证伪压力
> 来自另一条规则:**同一个元素被声明在两块屏上读出同一段文字,是一条 confusion**——
> 那正是「这段文字区分不出这两屏」。例外只有一个:两块屏都声明自己是**同一页**时不报,
> 因为一页拍两次不是混淆(连续滚动的网格拍两个滚动位置,标题当然一样)。屏为此多了一
> 个可选的「我是哪一页」,而这个声明**本身要被度量**:那一页必须在这块屏上解析成功。
> 判分侧的置信度与判决并排上报,`read_floor` 以下的读出仍然记为「没读到」。
