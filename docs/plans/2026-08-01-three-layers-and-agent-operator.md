...
> 融合修订过一轮,修订处一律以文内的 2026-08-01 注为准。
...
> **修订 2026-08-01**:层归属按[页面模型上移到脚本层](2026-07-31-script-owned-page-model.md)
> 修订——C++ 的原语面是 `cycle_match` / `cycle_read` / `cycle_click`,加输入原语与项目
> 文件读写;认页、找元素、同类枚举、语义等待都在第二层,踩着 `cycle_match` / `cycle_read`
> 实现。本文各节写的「底层事实」按**能力**归属读:那些事实确实存在、确实由 C++ 的原语
> 支撑,但它们住在哪一层以该文档为准。
...
> **修订 2026-08-01(执法者换人,规则不变)**:element 与 page 上移第二层之后,C++ 的
> 硬保证收敛成四条——hit 来自**本票据**的 `cycle_match`(同帧)、观察租约新鲜度、
> 指纹兼容、票据单次消费。「业务脚本只点标注元素」不消失,只是执法者从 C++ 变成
> **可信第二层**:裸点击原语以闭包 upvalue 只交给第二层,业务环境暴露的是「点元素 /
> 点容器内选中项」这类包装,裸点击**根本不在那张表上**;探索环境(Agent 前端)才装载
> 裸点击。这与上面第 2 条是同一条纪律,只是它现在也管住了坐标点击本身。
...
本文对[标注模型文档](2026-07-31-annotation-model-capabilities.md)追加四条裁决:
...
> **修订 2026-08-01**:标注模型的**实现位置**已按
> [页面模型上移到脚本层](2026-07-31-script-owned-page-model.md)上移第二层,模型的
> **设计结论**一条不改,上面四条裁决原样成立。裁决 4 的期待文字按当日裁决落地:
> 期待文字作为**数据**写进项目文件(所以可证伪、可重放),而「怎么算匹配」的核对逻辑
> 归第二层 Luau,C++ 的 `cycle_read` **不收** `expected` 参数。这与
> [标注模型文档](2026-07-31-annotation-model-capabilities.md) §四之二.3 并立成立,
> 不冲突:一个说事实住在哪,一个说规则归谁。术语:本节的「variant」与「状态元素」
> 按 §六 的同日注读作 appearance。
...
- **有名字的 appearance = 局部事实。**同一矩形上互斥的状态是**一个**元素带一张命名
  appearance 列表,不是 N 个元素。例:`speed_button` 的 `speed_1x / speed_2x /
  speed_3x`,`auto_button` 的 `auto_on / auto_off / auto_gray`。命中身份上到脚本面:
...
- **无名 appearance(曾称 variant)= 脚本永远不需要区分的外观差异。**例:back 按钮
  ...
  一个 appearance 最坏多一次搜索。

> **修订 2026-08-01(机制统一)**:本节采用[标注模型文档](2026-07-31-annotation-model-capabilities.md)
> §四之二.4(a) 的**有名字的 appearance**;同日一度裁下的「把互斥状态拆成各自独立的
> 元素(`speed_1x` / `speed_2x` / `speed_3x` 各成一个元素)」**已否决**。术语随之统一
> ——一律写 appearance(曾称 variant)。上一条那句代价也按 §四之二.4 收窄:**每周期搜
> N 次只落在状态读出这一类**;「哪套形态适用由页面决定」的那一类在引用侧钉死
> appearance,那一页每周期只搜 1 次。
...
标注过的东西」(执法者见 §二 的 2026-08-01 注);要不要点,由脚本先读
`hit.appearance` 是不是 `auto_gray` 再决定。
...
> **修订 2026-08-01**:本节是**能力面**——脚本能用的动词全集。物理归属(哪几个是 C++
> 原语、哪几个在第二层用 Luau 踩着原语实现)见
> [页面模型上移到脚本层](2026-07-31-script-owned-page-model.md)。

底层事实(运行模式可用):

- 认页:当前帧解析为哪个页面
- 找元素:本页引用的元素,命中与位置;验证方式按元素声明执行(模板匹配,或
  OCR 读出该矩形——「读出来的算不算匹配」的核对逻辑在第二层,见 §五 的
  2026-08-01 注),按需进行,不预扫全部矩形
...
1. 本文审定后,对照现有代码盘点:哪些直接是零件、哪些要改、哪些放弃。此前挂起的
   ...
   > **修订 2026-08-01**:复用/退役清单由
   > [页面模型上移到脚本层](2026-07-31-script-owned-page-model.md) §九 承担,本条的
   > 普查材料退居其后,作为退役时的爆炸半径参考。
...
3. 标注模型 v2 的动工顺序沿用其文档建议,动词清单以本文 §七为准。
   ...
   > **修订 2026-08-01**:迁移改按
   > [页面模型上移到脚本层](2026-07-31-script-owned-page-model.md)执行——模型上移
   > 第二层,项目文件格式由第二层定,原来那份「两个 schema 一起升版」的计划**作废**。
