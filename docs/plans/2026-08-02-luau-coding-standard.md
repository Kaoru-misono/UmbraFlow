...
### 1. `observe.luau:192` 的错误层级是错的(活 bug)
...
它下面二十行的兄弟 `readTarget` 用的是 level 3,注释(`observe.luau:210-212`)写着的正是
前者违反的那条规则。**全仓库没有任何测试断言过 error level**,所以 CI 看不见。
...
### 3. `mint.frozen_extra` 是浅冻,且通向存盘时的静默删字段

`mint.luau:106-115`,6 个调用点。`Element.new{ extra = { tags = {...} } }` 冻出来的元素,
它的 `extra.tags` 仍然是调用方那张可写的表——头部注释声称的「快照」在这里是假的。

更糟的是第二段:`project.encode` 的 `renderValue`(`project.luau:240`)把任何表都按数组渲染,
...
理由不是统一。`project.luau:77-162` 的 `k_sectionFields` 就是工程文件的合法键名表,
...
(`project.luau:753-757`),原始行照样往返写回磁盘,文件看着没问题,但 `project.build`
...
| `## Type aliases` / `## Type placement` / `## Namespaces` | 全部删除:这个 VM 里没有模块解析器,只有全局绑定(`project.luau:166-169`、`navigation.luau:75-81`),一个 chunk 根本无法命名另一个 chunk 的导出类型。取代它的是**两层可见性**——`local function lowerCamelCase` 是私有,`module.snake_case` 是公开,不能被够到的东西就不能挂在模块表上 |
...
`model.luau:85-88` 的常量和它在 `modules/vision/.../frame-analysis.hpp` 里的原件,
...
- **用局部变量遮蔽已发布模块名**。1 处注释声称这是规则,4 处在做。一比四不是约定。
  ...
  `regress.luau:460` 里遮蔽了 `hits` 的那处改名,要么删掉 `observe.luau:750-752`
  那段声称规则的注释。
...
- **报告类对象要不要冻**。`regress` 的 `Verdict`(`regress.luau:866`)是贯穿整个遍历的
...
| # | 改动 | 规模 | 为什么在这个位置 |
|---|---|---|---|
| 1 | `observe.luau:192` 的 `requireCtx` 改抛 level 3 | 1 处 | 唯一找到的活 bug。7 个公开 verb 目前都在指错行。它下面二十行的兄弟就写着这条规则 |
| 2 | 每个构造器补一个对抗性 freeze 测试 | 新增约 20 行 | 让现存 37 处 `table.freeze` 变成可依赖的。今天全删掉测试仍然绿 |
| 3 | `mint.frozen_extra` 递归冻,或 `check_extra` 拒绝嵌套表 | 1 个函数,6 个调用点 | 「值是快照」唯一为假的地方,且通向 `project.encode` 的存盘静默删字段 |
| 4 | `scripts/fix_format.py` 的 `TEXT_EXTENSIONS` 加 `.luau` | 1 行 | 全仓库最便宜的真实强制。`embed_luau.py:85` 遇到一个 CR 字节就硬失败,而目前没有任何东西规范化 |
| 5 | 删掉 `ctx.luau:78-79` 的两个 wait 默认值 | 2 行 | 已死(`modules/`、`entry/`、`tests/` 中零引用),且与 `observe.wait_until` 明确拒绝提供默认值的立场矛盾。留着早晚有人伸手去拿 |
| 6 | 冻 `scribe.measure` 的 Measurement 及其嵌套 `rect`/`key` | 1 个返回 + 2 处嵌套 | 堵上调用方在 `measure` 和 `author_element` 之间改写 `m.key.tolerance` 的洞;`scribe.luau:259-268` 已经写出了这条要保护的性质 |
...
| 10 | 检查器扩展 level 2/3 规则 | 约 70 行 | 只在 #1 之后才值得;需要按头部自身缩进列做 `end` 配对,否则报 4 个假阳性 |
...
| 14 | `regress.luau:460` 的 `hits` 改名,或删掉 `observe.luau:750-752` | 1 处 | 二选一:今天一段注释声称的规则,代码五处只守一处 |
...
1–7 合计约 40 行源码改动,关掉了找到的每一个具体失败。8–12 是门禁建设。
13–15 需要一次决定或一套新工具,不能并进前面几项。
