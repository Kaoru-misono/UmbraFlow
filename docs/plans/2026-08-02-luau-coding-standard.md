...
> **计数与行号的时效(2026-08-03 复核)**。测量做在 2026-08-02;当天稍后
> `cef4886`(矩形三选一)与 `0b5c1e5`(长按 + 空读裁决)又往
> `modules/task/runtime/` 里加了约 1350 行,于是本文所有**行号**在写下的当晚就已经
> 失效。行号已按当前 HEAD 重新钉过一遍,**聚合计数没有重测**——它们仍是 08-02 那次
> 普查的数字,不是今天的数字。已知偏差:`table.freeze` 当时 37 处、现在 41 处;
> `error()` 当时 248 处、现在 269 处;文件仍是 15 个,行数从 8907 涨到 10254。
> 第 2 节那个「全删掉仍然全绿」的证伪实验是在 37 处的状态下跑的,没有重跑。
...
### 1. `observe.luau:285` 的错误层级是错的(活 bug)
...
| # | 改动 | 规模 | 为什么在这个位置 |
|---|---|---|---|
| 1 | `observe.luau:285` 的 `requireCtx` 改抛 level 3 | 1 处 | 唯一找到的活 bug。7 个公开 verb 目前都在指错行。同文件 `readTarget`(330)就写着这条规则 |
...
| 4 | `scripts/fix_format.py` 的 `TEXT_EXTENSIONS` 加 `.luau` | 1 行 | 全仓库最便宜的真实强制。`embed_luau.py:91` 遇到一个 CR 字节就硬失败,而目前没有任何东西规范化 |
