...
| 无人值守 | 能做,可靠性由脚本自担 | 能做,受桌面状态影响 | ✅ 已有多个真实游戏项目验证 | ✅ 核心目标,但必须先通过长程稳定性验收 |
...
- **⚠ P0 承重取舍(重要)**:"整套每日全跑完"意味着多个不同每日任务 + 大概率随机弹窗 + 长程中断点。
  这会撞上原判给 P1 的 **弹窗 interrupt(D6)** 与 **跨任务复用(D7)**。**取舍:P0 允许"笨但能跑通"**——
  弹窗写死 `if`、多任务复制粘贴,**不强求 interrupt/子任务抽象**;抽象在 P1 回头重构。先拿到"每天真替我刷完"的价值。
- **交付顺序**:
  1. **P0-A 可视化标注系统**:独立 GUI 完成截图/样本管理 → 框选与属性编辑 → manifest/模板/page 生成 →
     runtime 同算法 Preview/Test → 静态截图回归。
  2. **P0-B 执行切片**:Luau runtime + Engine observe/act/wait 循环 + Fake Controller + trace +
     capability 沙箱/配额/硬取消 + generation 热加载。
  3. **P0-C 真实价值切片**:卡厄斯梦境页面目录与整套每日 Luau 脚本,真机无人值守跑完一轮。
- **退出标准**:
