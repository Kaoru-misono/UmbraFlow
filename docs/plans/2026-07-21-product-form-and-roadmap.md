...
- **宿主侧组装:等第二个真实组装根出现再动**(2026-08-15 裁定,依据见归档的
  [HostPlugin 架构提案](../archive/reviews/2026-08-14-host-plugin-architecture-proposal.md))。
  今天五个命令里只有 `explore` 和 `observe` 共享 provider 组装,共享部分已经是
  `platform::bindOcrEngine` 与 `platform::bindTarget` 两个函数,被组织的对象总共约 300 行。
  等 authoring 前端独立成自己的 front-end、或 P2 常驻宿主落地——那时才会同时存在两条组装路径——
...
该裁决已冻结,全文移至
[`docs/decisions/2026-07-21-luau-script-language.md`](../decisions/2026-07-21-luau-script-language.md)。
内容逐字不变;路线图不再持有它,因为路线图每次状态变化都会被重写。
