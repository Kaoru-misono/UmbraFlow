...
// Colour-key masking through the exploration surface: a crop cut under a key,
// the alpha plane it bakes, and the counts it reports. The scores are the test
// rather than the alpha bytes, because a mask that reaches the PNG but never
// reaches the matcher would satisfy any assertion about pixels -- so each case
// cuts one template two ways from the same pixels and compares what the matcher
// reports.
namespace uf::task
{
    namespace
