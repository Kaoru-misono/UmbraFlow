...
namespace uf::input_agent
{
    ...
    TEST_CASE("input-agent parses every bounded command operation")
    {
    ...
    TEST_CASE("input-agent parses a key command over every supported family")
    {
    ...
    TEST_CASE("input-agent parses a scroll command with its wheel delta")
    {
    ...
    TEST_CASE("input-agent rejects scroll commands it could not deliver")
    {
    ...
    TEST_CASE("input-agent key result line reports delivery separately from ok")
    {
    ...
    TEST_CASE("input-agent rejects unknown key names and misplaced key fields")
    {
    ...
    TEST_CASE("input-agent key frames must be fresh non-aliasing files")
    {
    ...
    TEST_CASE("input-agent rejects settle times above the bounded wait")
    {
    ...
    TEST_CASE("input-agent rejects malformed UTF-8 command strings")
    {
    ...
    TEST_CASE("input-agent confines output paths to its canonical directory")
    {
        auto const token = std::chrono::steady_clock::now()
            .time_since_epoch()
            .count();
    ...
    TEST_CASE("input-agent queue reader preserves incremental line framing")
    {
    ...
    TEST_CASE("input-agent queue reader rejects truncation")
    {
    ...
    TEST_CASE("input-agent queue reader bounds unterminated commands")
    {
    ...
    TEST_CASE("input-agent resumes a restarted queue instead of replaying it")
    {
    ...
    TEST_CASE("input-agent will not guess a start for an uncursored queue")
    {
    ...
    TEST_CASE("input-agent queue cursor refuses every partial write")
    {
    ...
    TEST_CASE("input-agent queue cursor refuses a foreign or unreadable position")
    {
    ...
    TEST_CASE("input-agent file writer validates the opened output path")
    {
    ...
    TEST_CASE("input-agent clears per-command audit state across many clicks")
    {
        auto audit          = AuditLog{};
    ...
    TEST_CASE("input-agent click coordinates fail closed at client bounds")
    {
    ...
    TEST_CASE("input-agent click validation rejects a stale generation")
    {
