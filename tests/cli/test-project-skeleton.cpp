...
// does not have them. Absence is the half that matters: task::ProjectFileStore
// deliberately refuses a name whose parent is not there, and nothing else in the
// product may create one -- the store proves confinement by canonicalizing a
// parent that really exists, and the script layer above it has no directory verb.
namespace uf::cli
{
    namespace
    {
        ...
        // doctest evaluates a *_MESSAGE argument whether or not the assertion
        // holds, so reaching into `error()` unconditionally would trip on a pass.
        [[nodiscard]]
    ...
    TEST_CASE("laying out a skeleton twice changes nothing the first run made")
    {
        ...
        // A second run that recreated rather than checked would silently destroy
        // an authored template asset already sitting there.
