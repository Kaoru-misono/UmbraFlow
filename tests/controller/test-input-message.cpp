...
TEST_CASE("pointer lParam packs y high and x low")
{
    ...
    CHECK(
        uf::controller_detail::pointerLParamBits(
            pixel(
                std::numeric_limits<uf::int16>::max(),
                std::numeric_limits<uf::int16>::max()
            )
        ) == 0x7FFF'7FFFU
    );
...
TEST_CASE("key specs carry the virtual key in wParam and flags in lParam")
{
    ...
    CHECK(down.m_message == uf::controller_detail::g_wmKeyDown);
    CHECK(down.m_wParam == 0x41U);
    CHECK(down.m_lParam == uf::intptr{0x001E'0001});
    ...
    CHECK(up.m_message == uf::controller_detail::g_wmKeyUp);
    CHECK(up.m_lParam == uf::intptr{0xC01E'0001});
}
...
TEST_CASE("numpad Enter preserves its extended bit across down and up")
{
    ...
    CHECK(down.m_lParam == uf::intptr{0x011C'0001});
    CHECK(up.m_lParam == uf::intptr{0xC11C'0001});
}
...
TEST_CASE("pointer specs set the button mask only while down")
{
    ...
    CHECK(
        uf::controller_detail::pointerSpec(
            ...
            .m_message = uf::controller_detail::g_wmMouseMove,
            .m_wParam = 0,
    ...
    CHECK(
        uf::controller_detail::pointerSpec(
            ...
            .m_message = uf::controller_detail::g_wmMouseMove,
            .m_wParam = uf::controller_detail::g_leftButtonMask,
            .m_lParam = 0x00C8'0064,
        }
    );
    CHECK(
        uf::controller_detail::pointerSpec(
            ...
            .m_message = uf::controller_detail::g_wmLeftButtonDown,
            .m_wParam = uf::controller_detail::g_leftButtonMask,
            .m_lParam = 0x00C8'0064,
        }
    );
    CHECK(
        uf::controller_detail::pointerSpec(
            ...
            .m_message = uf::controller_detail::g_wmLeftButtonUp,
            .m_wParam = 0,
            .m_lParam = 0x00C8'0064,
        }
    );
...
TEST_CASE("character and Unicode-character specs post code points directly")
{
    CHECK(
        uf::controller_detail::charSpec(0x0041U) == uf::controller_detail::PostSpec{
            .m_message = uf::controller_detail::g_wmChar,
            .m_wParam = 0x41U,
            .m_lParam = 1,
        }
    );
    CHECK(
        ...
        == uf::controller_detail::PostSpec{
            .m_message = uf::controller_detail::g_wmUnichar,
...
TEST_CASE("UTF-8 text follows the WM_CHAR UTF-16 code-unit path")
{
    ...
    CHECK(
        *codeUnits == std::vector<uf::uint16>{
            0x0041U,
            0xD83DU,
            0xDE00U,
        }
...
{
    for (auto const virtualKey : std::array<uf::uint16, 16>{
        0x0025U,
        0x0026U,
        0x0027U,
        0x0028U,
    ...
    for (auto const virtualKey : std::array<uf::uint16, 3>{0x0041U, 0x0030U, 0x000DU})
    {
...
TEST_CASE("delivery to an invalid window audits before reporting disconnect")
{
    ...
    CHECK(audit.records()[0].m_message == uf::controller_detail::g_wmChar);
    CHECK(audit.records()[0].m_wParam == 0x41U);
    CHECK(audit.records()[0].m_target == uf::uintptr{0xDEAD'BEEF});
}
