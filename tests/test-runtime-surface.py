...
# The readings shape had a second writing in C++ until the framework stopped
# interpreting a Project's state: modules/deployment embedded the resolver's
# `state_readings` in the derive-input schema it compiled, and a rule here
# derived one from the other so neither could be edited alone. That schema went
# with the fold it validated for, so there is no second writing left to compare
# and the rule that compared them is gone rather than pointed at something else.
