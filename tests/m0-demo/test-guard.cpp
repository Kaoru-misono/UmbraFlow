...
TEST_CASE("m0 integrity labels follow Windows RID bands")
{
    ...
    {
        CHECK(uf::m0_demo::IntegrityLevel::fromRid(testCase.m_rid).label() == testCase.m_label);
    }
}
...
TEST_CASE("m0 guard mode compares foreground and cursor")
{
    ...
    auto const movedCursor = uf::m0_demo::GuardBaseline{0x10, {7, 6}};
    auto const changedForeground = uf::m0_demo::GuardBaseline{0x20, {5, 6}};
    CHECK_FALSE(
        uf::m0_demo::checkGuard(
            policy,
            targetWindow,
            baseline,
            movedCursor
...
{
    auto const policy = uf::m0_demo::GuardPolicy::forMode(uf::m0_demo::Mode::Coexist);
    auto const baseline = uf::m0_demo::GuardBaseline{0x10, {5, 6}};
    auto const different = uf::m0_demo::GuardBaseline{0x99, {100, 200}};
    auto const check = uf::m0_demo::checkGuard(policy, 0x10, baseline, different);
