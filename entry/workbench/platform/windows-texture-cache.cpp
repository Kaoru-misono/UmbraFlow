...
namespace uf::workbench::platform
{
    ...
    {
        for (auto const& cached : m_cache->textures)
        {
            if (cached.id == asset.id)
            {
                return GpuSourceTexture{
                    ...
                    .textureHandle = std::bit_cast<uint64>(cached.view.get()),
                    .width         = cached.width,
        ...
        UF_TRY_VALUE(
            decoded,
            image::decodePng(asset.pngBytes, "workbench-canvas-source.png")
        );
        ...
        initialData.pSysMem     = decoded.pixels.data();
        ...
        auto const textureResult = m_cache->device->CreateTexture2D(
            &description,
            &initialData,
            texture.put()
        );
        ...
        auto const viewResult = m_cache->device->CreateShaderResourceView(
            texture.get(),
            &viewDescription,
            view.put()
        );
