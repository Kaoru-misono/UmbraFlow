...
{"file": "entry/cli/run-windows.cpp", "start": 107,
"caption": "组合根：包装适配器，创建会话，跑主流程",
...
"ann": [
(109, "WgcFrameSource 和 ControllerActionSink 是仅有的两个平台适配器，各自只有几十行转发代码"),
(115, "指定初始化器让配置一目了然：识别预算、超时、帧龄上限、取消令牌"),
(120, "这个 stop_token 来自 Ctrl-C 处理，会一路传到识别循环深处（第 7 章）"),
(126, "三个接口用 unique_ptr 交出所有权，接口实现和其中的平台资源跟会话同生共死"),
(137, "findAction 是 pageWait.observation 的方法：查找按钮只能在认出页面的同一帧上进行（第 4 章）")]}], "asides": []},
{"h": "两个值得注意的细节", "paras": [
"结尾的代码有两个细节。第一，findAction 没找到按钮时返回的是空的 optional，程序正常写报告退出。没找到是合法的业务结果，不走错误处理——什么算业务结果、什么算错误，这个区分在第 7 章讲。第二，act 的第一个参数写的是 std::move(pageWait.observation)：点击会把整个观察对象消耗掉，之后这个变量就不能再用了。为什么要这样设计，是第 4 章的主题。"], "code": [
{"file": "entry/cli/run-windows.cpp", "start": 150,
...
"code": r"""        UF_TRY_VALUE(
        ...
        return report;""",
"ann": [
(153, "move 进去之后，这个 observation 就不能再用了——编译期和运行时都有检查"),
(158, "返回的 ActReceipt 记录点击对应的帧和实际投递的坐标，用来写报告")]}], "asides": [
...
"sections": [
{"h": "FrameSource：截图，加一个看似多余的方法", "paras": [
"FrameSource 有两个方法。capture 返回一帧画面，没什么可说的。validateTargetInstance 检查绑定的还是不是原来那个窗口——乍一看多余，capture 失败不也能发现窗口没了吗？它单独存在的原因要到第 5 章才完全展开：在点击发出前的最后一刻，需要一次不截图的快速复核。注释里还写明了这个接口预留的两个扩展点：将来的非 Windows 平台，以及 CI 里回放固定画面序列的测试替身。"], "code": [
{"file": "modules/engine/source/engine/ports.hpp", "start": 13,
"caption": "FrameSource：注释里写清了接口存在的理由",
...
{"h": "ActionSink：把安全要求写进参数列表", "paras": [
"ActionSink 的注释是三个接口里语气最重的。click 除了坐标还要接收一个 lease（观察租约，第 5 章的主角），并且注释用大写的 MUST 要求实现把它原样传给投递层。为什么点击接口要带租约？因为授权分两层：engine 这边授权完是第一层，controller 在真正发消息之前，还要用 lease 里的帧身份和年龄再独立查一遍，这是第二层。如果接口只收坐标，写适配器的人少传一个参数，第二层检查就悄悄消失了，而且没有任何报错。把 lease 写进函数签名之后，这个错误就没法犯了。"], "code": [
{"file": "modules/engine/source/engine/ports.hpp", "start": 38,
"caption": "ActionSink：lease 必须原样传递，这是接口的一部分",
...
{"h": "TraceSink：写记录也可能失败", "paras": [
"第三个接口 TraceSink 只有一个 emit 方法，但注意它的返回值是 Status：写记录可能失败，而且失败会中止当前操作，不是打条警告继续跑。注释给了理由：可追溯性是这个系统承重的约束之一，所以 engine 在错误发生的那一刻就记录，赶在任何上层调用者有机会吞掉错误之前。这条证据链的完整设计放在第 8 章。"], "code": [
{"file": "modules/engine/source/engine/ports.hpp", "start": 67,
"caption": "TraceSink：记录失败是错误，不是警告",
...
"Observation 的思路是把一次观察变成一个对象：画面、租约、身份信息都装在里面，由 EngineSession::observe 统一发出。想识别页面或者找按钮，只能调它的方法，读到的自然是同一帧画面；想点击，必须把整个对象 move 进 act，之后原变量就不能再用了。"], "code": [
{"file": "modules/engine/source/engine/session.hpp", "start": 72,
"caption": "Observation 的完整声明，注释里有生命周期证明",
...
"ann": [
(88, "构造函数私有，加 friend：只有 EngineSession::observe 能创建观察对象"),
(92, "这里用裸指针是合理的，注释给了证明：会话一定比它发出的观察活得久，指针只是引用，不管理生命周期"),
(103, "拷贝构造删除：两个句柄指向同一帧画面，就意味着可能点两次"),
(104, "移动构造会把源对象标记为失效，moved-from 的句柄和用过的句柄行为完全一样"),
(111, "识别页面和找按钮都是成员方法，同帧决策不是靠自觉，是唯一能写出来的形式")]}], "asides": []},
{"h": "类型挡住大多数，标志位兜底", "paras": [
"act 按值消耗句柄之后，大多数误用在编译期就结束了。但 C++ 的 move 不是真正的线性类型：moved-from 的对象还在，如果别处还留着引用，仍然可能被调用。所以每个方法进来的第一件事是检查 m_invalidated 标志，发现句柄已经用过就返回 StaleObservation 错误。两道防线配合下来：编译器拦住绝大多数误用，标志位把剩下的变成一个确定的运行时错误。中间没有碰巧还能用的灰色地带。",
...
{"file": "modules/engine/source/engine/session.cpp", "start": 429,
...
"ann": [
(433, "已经请求取消的运行绝不投递输入"),
(445, "拿别的会话的观察来点击被归为程序错误（InternalInvariant），和可恢复的运行时错误分开"),
(453, "第 4 章的失效标志，在这里第一次被用作守卫")]}], "asides": []},
{"h": "第一层授权", "paras": [
"过了前三道门，engine 从观察对象里取出帧身份，交给 annotation 模块授权。授权函数的参数列表本身就是一份检查清单：识别目录（查这个动作允许出现在哪些页面上）、已解析的页面（证据一）、动作检测结果（证据二）、观察租约（保证画面够新鲜）、当前投递状态（当前项目指纹和当前时间）。核心的比对是身份：页面证据、动作证据、即将执行的点击，三者的 SessionId、TargetGeneration、FrameId 必须完全一致——换句话说，它们必须来自同一个捕获会话、同一代目标窗口、同一帧画面。",
...
{"file": "modules/domain/source/domain/detection.hpp", "start": 47,
...
"ann": [
(49, "SessionId 区分捕获会话；TargetGeneration 在窗口实例、句柄或客户区尺寸变化时加一；FrameId 逐帧递增"),
(52, "租约有过期时间：画面证据是会过期的，太旧的观察不能拿来兑换点击"),
(70, "构造函数私有，只能通过 forFrame 从真实的帧构造，凭空造不出有效租约"),
(84, "validate 四项全比，任何一项不符就返回 StaleObservation")]}], "asides": []},
...
{"file": "modules/engine/source/engine/session.cpp", "start": 486,
...
"ann": [
(489, "坐标换算用的是那一帧自带的变换信息，不是当前的窗口位置"),
(495, "第 3 章说的第二次 validateTargetInstance 就在这里：目标被换掉的话，click 一次都不会被调用"),
(508, "click 带着 lease，controller 的第二层校验从这里开始"),
(514, "关键的一行：点击已经发出，先让句柄失效，再做后面可能失败的记录。顺序反了就有双击风险")]}], "asides": [
...
"Space 是个空结构体，编译完就没了：Point<FrameSpace> 的内存布局和两个裸 float 完全一样，没有任何运行时开销。但在类型系统里，Point<FrameSpace> 和 Point<ClientSpace> 是两个不相干的类型。回头看第 5 章的换算链：pixelPointToFramePoint 返回帧坐标，frameToClient 返回客户区坐标，ActionSink::click 只收 Point<ClientSpace>。想把没换算过的帧坐标直接传给点击，编译不过。"], "code": [
...
{"h": "ID 也有同样的问题", "paras": [
"SessionId、FrameId、TargetGeneration 底层都是整数，而第 5 章的授权恰恰要在这三个数之间做比对。如果它们都用 uint64，把参数顺序传反的代码不但能编译，多数测试还发现不了——测试场景里这几个值经常恰好相等。等真出问题的时候，就是授权拿错值做比对、放行了不该放行的点击。StrongId 用空标签类型把每种 ID 隔成独立类型，传反了直接编译报错："], "code": [
