...
namespace uf::annotation
{
    ...
    // One authored element compiles to exactly one CompiledElement, under the
    // element's own id, however many pages reference it and however many
    ...
    // compiled form under its own id at all -- so a page signature naming that
    // element pointed at nothing in the runtime catalog. One compiled element
    // per authored element is also what the capability merge buys: a patch of
    // pixels that both names its page and can be clicked is searched once per
    // cycle, and compiling one per page would put the second search straight
    // back. The element id is the one name scripts, authorisation, and the
    // trace all use, so it is the one the runtime catalog must answer to.
    //
