...
namespace uf::cli
{
    ...
    auto observeUsageText() noexcept -> std::string_view
    {
        return
            ...
            "                               installed RuntimeArtifact and, if this\n"
            "                               Operator wrote one, policy-artifact.json\n"
            "  --ocr-models DIR             Directory holding\n"
            "                               ppocr-v6-small-rec and\n"
            "                               ppocr-v6-small-det\n"
            "\n"
    ...
    auto invokeUsageText() noexcept -> std::string_view
    {
        return
            ...
            "caller's behalf.\n"
            "\n"
            "The kind is only half of who may reach the machine surface. The\n"
            "other half is this machine's own: the PolicyArtifact at\n"
            "policy-artifact.json inside --runtime lists every Tool it permits\n"
            "on the Privileged surface, and admission refuses a top-level call\n"
            "of a privileged-surfaced Tool that list does not name, whichever\n"
            "actor presents it. A project declares `surface` about its own\n"
            "Tools and that declaration grants nothing. An absent artifact is\n"
            "the Operator deny-all: no effect is allowed and no machine\n"
            "surface is opened.\n"
            "\n"
