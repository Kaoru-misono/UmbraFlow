...
namespace uf::task
{
    // The largest PNG blob one probe may decode. A crop of one screen is a few
    // hundred kilobytes at most, so this is the same ceiling template_load uses:
    // without a bound, the memory one call costs would be a property of whatever
    // the caller happened to hand it.
    ...
    // far a pixel may sit from it and still count. It is optional at the call site
    // (see probePngRegion) because the first probe an agent runs is the one that
    // has no key yet -- "what colours are in this rectangle" is how a key gets
    // chosen.
    //
    ...
    // because a key an agent probed with and a key it then cut with have to be the
    // same key or the counts the two verbs report stop being about one
    // measurement.
    ...
    //
    // It does not carry v4 `frames probe`'s `masked_mean_gray_spread` or
    // `rect_mean_gray_spread`. Both are the mean, over a rect, of how far a
    // pixel's grey moved BETWEEN FRAMES, and this verb takes one blob, so both
    // would read zero on every call. An agent that wants them crops the same rect
    // twice and compares.
    //
    // The census half is folded in because the loop is "crop, then probe to fix
    ...
    struct PixelProbeReport final
    {
        ...
        // Absent when the caller supplied no key, rather than zero: "nothing was
        // selected" and "no selection was asked for" are different answers.
    ...
    // safe as a primitive of its own rather than a cycle verb -- the bytes are
    // already in the caller's hand, and measuring them establishes nothing new
    // about the live target.
    //
    // A rect the decoded image does not contain is refused rather than clamped: a
    // probe partly off the image would report counts a caller would read as being
    // about the region they asked for.
    [[nodiscard]]
    auto probePngRegion(
