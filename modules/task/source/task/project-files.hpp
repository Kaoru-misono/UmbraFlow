...
namespace uf::task
{
    ...
    // above any page model a project writes -- the largest annotation document
    // this repository has ever produced is measured in tens of kilobytes -- and
    ...
    // rest of the disk. It is the same shape as the input agent's output
    // confinement (entry/input-agent/path-validation.*), reimplemented here
    // rather than shared because the two have opposite freshness rules: an agent
    // capture must be a file that does not exist yet, while a project file is
    // rewritten in place every time the model changes.
    //
