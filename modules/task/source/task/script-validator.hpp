...
namespace uf::task
{
    ...
    // one canonical resource spelling -- umbra.recognizers.<name> and
    // umbra.pages.<name> direct literal member access -- plus umbra:<verb>(...)
    // method calls, and that every named resource resolves against `surface`.
    // This is the S0 resource closure (annotation-design 4): every reference is
    // enumerated and closed here, so a missing resource is caught before the VM
    // exists; runtime nil remains only defense in depth.
    //
